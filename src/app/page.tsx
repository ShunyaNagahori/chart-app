...
export default function Home() {
  ...
  const handleValueSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (!!textInput.trim()) {
      setValues({...values, [textInput.trim()]: numberInput});
      ...
      if (dataCount === 2) {
        setSecondValues({...secondValues, [textInput.trim()]: secondNumberInput});
  ...
  return (
    <main className='w-screen'>
      <div className='flex'>
        <div className='p-2 space-y-2 w-2/3 border-r'>
        ...
        <div className='p-2 space-y-3 w-1/3'>
          {title == 'タイトル未登録' ? (
            <div>
          ...
          <div>
            ...
            { dataCount === 1 && (
              <form onSubmit={handleValueSubmit}>
                ...
                <div className='mb-2'>
                  <input type='text' className='border-gray-300 border px-1 outline-none w-full block' value={textInput} onChange={handleTextInputChange} placeholder='ラベル名' required />
                </div>
                <span className='flex justify-center mt-2'>
